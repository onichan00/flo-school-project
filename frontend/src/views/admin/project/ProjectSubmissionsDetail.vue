  ...
  <div class="container mx-auto px-10 py-4">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div class="p-4 shadow-md rounded-md border border-gray-200 text-left">
        <p>{{ dataObject.name }}</p>
        <p>{{ clientOfThisProject.company }}</p>
        <p>{{ specialistFullName(clientOfThisProject) }}</p>
        <p>{{ formatDate(dataObject.created) }}</p>
      </div>
    ...
    <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <div class="p-6 space-y-6">
          <label for="availableSpecialists" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">Select an specialist</label>
          <select id="availableSpecialists"
            ...
            <option v-for="specialist in availableSpecialists" :key="specialist.id">{{ specialistFullName(specialist) }}</option>
          </select>
        </div>
        <!-- Modal footer -->
        <div class="flex items-center p-6 space-x-2 rounded-b border-t border-gray-200 dark:border-gray-600">
          <button data-modal-toggle="addSpecialistModal" type="button"
            class="text-white bg-florijnOrange hover:bg-florijnOrange-300 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Add</button>
          <button data-modal-toggle="addSpecialistModal" type="button"
